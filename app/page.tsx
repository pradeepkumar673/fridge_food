...
export default function HomePage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Recipes section scroll animation
      gsap.fromTo(
  ...
  const scrollToRecipes = () => {
    ...
    if (recipesSection) {
      gsap.to(window, {
        duration: 1.2,
        scrollTo: { y: recipesSection, offsetY: 80 },
        ease: "power3.inOut",
      })
    }
  }
      ...
      <div className="flex min-h-screen w-full">
        ...
        <main className="flex-1">
          <div className="sticky top-0 z-10 flex h-14 items-center gap-4 border-b bg-background/80 px-4 backdrop-blur-sm lg:hidden">
            <SidebarTrigger />
            <h1 className="font-display text-lg font-semibold">Fridge Feast</h1>
          </div>

          {/* Hero Section */}
          <section
            ref={heroRef}
            className="relative flex min-h-[90vh] items-center justify-center overflow-hidden bg-gradient-to-br from-primary/10 via-secondary/5 to-accent/10"
          >
            <Hero3D />
            <div className="relative z-10 mx-auto max-w-5xl px-6 text-center">
              <h1
                ref={titleRef}
                className="font-display mb-6 text-6xl font-bold leading-[1.1] tracking-tight text-balance md:text-7xl lg:text-8xl"
              >
                What's in your{" "}
              ...
              <p
                ref={subtitleRef}
                className="mb-10 text-xl leading-relaxed text-muted-foreground md:text-2xl text-pretty"
              >
              ...
              <div ref={buttonRef}>
                <Button
                  size="lg"
                  className="gap-2 rounded-full bg-gradient-to-r from-primary to-secondary px-8 py-6 text-lg font-medium shadow-lg transition-all hover:scale-105 hover:shadow-xl"
          ...
          {/* Recipes Grid */}
          <section id="recipes" className="px-6 py-20">
            <div className="mx-auto max-w-7xl">
              <div ref={recipesHeaderRef} className="mb-12">
                <h2 className="font-display mb-3 text-5xl font-bold">Recipes for You</h2>
